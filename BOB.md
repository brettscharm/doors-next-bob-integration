...
2. Wait for approval → call `create_baseline` (e.g., "Pre-V2 Import Baseline")
3. Confirm the baseline was created before proceeding

**Then proceed with the diff:**

1. Read the new PDF version locally
2. Read the existing requirements from the DNG module using `get_module_requirements` — **note each requirement's URL**
...
6. Only after explicit approval:
   - Call `update_requirement` for each changed requirement (pass the requirement URL from step 2 + new content)
...
1. Call `list_baselines` to show available baselines
2. The user picks a baseline to compare against
3. Read the current requirements using `get_module_requirements`
4. Tell the user:
   > "I can see the current requirements. The baseline '[name]' was created on [date]. Since I can't read from a specific baseline configuration via the API, here's what I recommend: open DNG and use the built-in 'Compare' feature to diff the baseline against the current stream — it will show you exactly which artifacts changed, were added, or were removed."
5. Alternatively, if the user just re-imported a PDF, you already showed them the diff table — reference that.
...
2. **Phase 2:** Follow Step 3d (Create Tasks) — using the just-created requirement URLs
3. **Phase 3:** Follow Step 3e (Create Test Cases) — using the same requirement URLs
